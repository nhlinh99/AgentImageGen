...
class QueueService(BaseServiceSingleton):
    ...
    def resolve_job_queue_state(
        ...
        try:
            ...
            self.logger.error('Error resolving queue state for job %s: %s', job_id, str(e))
            return (None, None, None)
    ...
    def lane_queue_job_infos_prefix(
        ...
        try:
            ...
            self.logger.error('Error reading lane queue prefix (%s, %s, end=%s): %s', queue_type, lane, end_rank_inclusive, str(e))
            return []
    ...
    def count_user_jobs_all_lanes(self, user_id: str) -> Dict[QueueLane, Dict[str, int]]:
        ...
        try:
            ...
            self.logger.error('Error counting user jobs all lanes: %s', str(e))
            return out
    ...
    def add_job_to_queue(
        ...
        try:
            ...
            self.logger.info('Job %s added to %s waiting queue at lane position %s', job_id, resolved_lane, queue_position)
            return queue_position

        except Exception as e:
            self.logger.error('Error adding job to queue: %s', str(e))
            raise
    ...
    def get_next_job_for_scheduler(
        ...
        try:
            ...
            self.logger.error('Error in get_next_job_for_scheduler: %s', str(e))
            return None
    ...
    def get_queue_position(self, job_id: str) -> Optional[int]:
        ...
        try:
            ...
            self.logger.error('Error getting queue position: %s', str(e))
            return None
            ...
            for lane in ALL_LANES:
                ...
                for job_data, _score in jobs_with_scores:
                    ...
                    if job_data_str.startswith(f"{job_id}:"):
                        ...
                        if removed:
                            self.logger.info('Job %s removed from %s waiting queue', job_id, lane)
                            return True
            return False
        except Exception as e:
            self.logger.error('Error removing job from queue: %s', str(e))
            return False

    def get_queue_length(self, service_type: Optional[str] = None) -> int:
        try:
            if service_type:
                total = 0
                for lane in ALL_LANES:
                    ...
                    for job_data, _score in jobs_with_scores:
                        job_data_str = _decode_redis_str(job_data)
                        _jid, st, _u = parse_queue_zset_member(job_data_str)
                        if st == service_type:
                            total += 1
                return total
            return (
                self.redis_client.zcard(self.queue_key_fast)
                + self.redis_client.zcard(self.queue_key_medium)
                + self.redis_client.zcard(self.queue_key_slow)
            )
        except Exception as e:
            self.logger.error('Error getting queue length: %s', str(e))
            return 0
    ...
    def get_queue_statistics(self) -> Dict[str, Any]:
        try:
            ...
            self.logger.error('Error getting queue statistics: %s', str(e))
            return {}

    def clear_queue(self, service_type: Optional[str] = None) -> bool:
        try:
            if service_type:
                removed_count = 0
                for lane in ALL_LANES:
                    ...
                    for job_data, _score in jobs_with_scores:
                        ...
                        _jid, st, _u = parse_queue_zset_member(job_data_str)
                        if st == service_type:
                            self.redis_client.zrem(key, job_data)
                            removed_count += 1
                self.logger.info('Cleared %s jobs from %s queues', removed_count, service_type)
                return True
            self.redis_client.delete(self.queue_key_fast)
            self.redis_client.delete(self.queue_key_medium)
            self.redis_client.delete(self.queue_key_slow)
            self.logger.info("Cleared all waiting queues")
            return True
        except Exception as e:
            self.logger.error('Error clearing queue: %s', str(e))
            return False
    ...
    def get_jobs_in_queue(
        ...
        try:
            ...
            self.logger.error('Error getting jobs in queue: %s', str(e))
            return []
    ...
    def lock_job(self, job_id: str, priority: JobPriority, service_type: str, lock_duration: int = 3600) -> bool:
        try:
            if self.is_job_locked(job_id):
                self.logger.info('Job %s is already locked', job_id)
                return False
                ...
                for job_data, score in jobs_with_scores:
                    ...
                    if job_data_str.startswith(f"{job_id}:"):
                        ...
                        if removed:
                            ...
                            self.logger.info('Job %s locked (%s) for %ss', job_id, lane, lock_duration)
                            return True

            self.logger.warning('Job %s not found in queue to lock', job_id)
            return False
        except Exception as e:
            self.logger.error('Error locking job %s: %s', job_id, str(e))
            return False

    def unlock_job(self, job_id: str) -> bool:
        try:
            if not self.is_job_locked(job_id):
                self.logger.info('Job %s is not locked', job_id)
                return False

            locked_raw = self.redis_client.hget(self.locked_jobs_key, job_id)
            if not locked_raw:
                self.logger.warning('No locked job info found for %s', job_id)
                return False
            ...
            self.logger.info('Job %s unlocked to %s queue at position %s', job_id, lane, pos)
            return True
        except Exception as e:
            self.logger.error('Error unlocking job %s: %s', job_id, str(e))
            return False
    ...
    def add_job_to_processing_queue(self, job_id: str, service_type: str, lane: QueueLane, user_id: str) -> bool:
        try:
            ...
            self.logger.info('Job %s (%s) added to %s processing queue', job_id, service_type, lane)
            return True
        except Exception as e:
            self.logger.error('Error adding job to processing queue: %s', str(e))
            return False
            ...
            for ln in lanes_to_search:
                ...
                for job_data, _score in jobs_with_scores:
                    ...
                    if job_data_str.startswith(f"{job_id}:"):
                        ...
                        if removed:
                            self.logger.info('Job %s removed from %s processing queue', job_id, ln)
                            return True
            self.logger.warning('Job %s not found in processing queues', job_id)
            return False
        except Exception as e:
            self.logger.error('Error removing job from processing queue: %s', str(e))
            return False

    def count_user_lane_jobs(self, user_id: str, lane: QueueLane) -> Dict[str, int]:
        """Count ZSET members in ``lane`` waiting/processing with user_id as third segment."""
        try:
            row = self.count_user_jobs_all_lanes(user_id)[lane]
            return {"waiting": row["waiting"], "processing": row["processing"]}
        except Exception as e:
            self.logger.error('Error counting user lane jobs: %s', str(e))
    ...
    def get_processing_job_order(self, job_id: str) -> Optional[int]:
        ...
        try:
            ...
            self.logger.error('Error getting processing job order: %s', str(e))
            return None
    ...
    def get_processing_jobs_count(self, lane: Optional[QueueLane] = None) -> int:
        try:
            ...
            self.logger.error('Error getting processing jobs count: %s', str(e))
            return 0
