...
def read_image_from_url(url: str, method: str = "opencv") -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.error('Failed to read image from URL %s: %s', url, str(e))
        return None
...
def read_image_url_opencv(url: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if image is None:
            logger.error('Failed to decode image from URL: %s', url)
            return None

        return image

    except Exception as e:
        logger.error('OpenCV method failed for URL %s: %s', url, str(e))
        return None
...
def read_image_url_pil(url: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.error('PIL method failed for URL %s: %s', url, str(e))
        return None
...
def read_image_url_requests(url: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.error('Requests method failed for URL %s: %s', url, str(e))
        return None
...
def read_image_from_path(file_path: str, method: str = "opencv") -> Optional[np.ndarray]:
    ...
    try:
        ...
        if method.lower() == "opencv":
            ...
            if image is None:
                logger.error('Failed to read image from path: %s', file_path)
                return None
            return image
        ...
        logger.error('Failed to read image from path %s: %s', file_path, str(e))
        return None
...
def normalize_image(image: np.ndarray, normalize_type: str = "0-1") -> np.ndarray:
    """
    Normalize image values.

    Args:
        image: numpy array of the image
        normalize_type: "0-1" for [0,1] range, "255" for [0,255] range
...
def batch_read_images_from_urls(urls: list, method: str = "opencv") -> list:
    """
    Read multiple images from URLs.

    Args:
        urls: List of image URLs
        method: Method to use for reading images

    Returns:
        List of numpy arrays (None for failed images)
    """
    images = []
    for url in urls:
...
def test_local_image():
    ...
    for method in ["opencv", "pil"]:
        print(f"  Method: {method}")
        image = read_image_from_path(local_path, method)
