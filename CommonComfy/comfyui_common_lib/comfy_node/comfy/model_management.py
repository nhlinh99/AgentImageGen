...
def cuda_device_context(device):
    ...
    if not isinstance(device, str) and device.type == "cuda" and device.index is not None:
...
def load_models_gpu(models, memory_required=0, force_patch_weights=False, minimum_memory_required=None, force_full_load=False, memory_target_load=None):
    cleanup_models_gc()
    global vram_state
    ...
    for loaded_model in models_to_load:
        ...
        if memory_target_load is not None:
            lowvram_model_memory = min(lowvram_model_memory, memory_target_load)
